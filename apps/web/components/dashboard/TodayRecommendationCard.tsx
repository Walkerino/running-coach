...
export function TodayRecommendationCard({ recommendation }: { recommendation: TodayRecommendation }) {
  return (
    <section className="relative overflow-hidden rounded-3xl bg-[#0f67fe] p-5 text-white shadow-[0_20px_40px_rgba(15,103,254,0.28)] lg:min-h-[360px] lg:p-6">
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-4 max-w-xl text-4xl font-extrabold tracking-[-0.05em] lg:text-5xl">{recommendation.title}</h2>
        </div>
      ...
      <div className="relative mt-8 grid gap-3 md:grid-cols-3">
        <div className="rounded-2xl bg-white/15 p-4 backdrop-blur">
          ...
          <p className="mt-1 text-base font-extrabold">{recommendation.workoutType}</p>
        </div>
        <div className="rounded-2xl bg-white/15 p-4 backdrop-blur">
          ...
          <p className="mt-1 text-base font-extrabold">{recommendation.durationMinutes ? `${recommendation.durationMinutes} min` : "As needed"}</p>
        </div>
        <div className="rounded-2xl bg-white/15 p-4 backdrop-blur">
          ...
          <p className="mt-1 text-base font-extrabold">{[recommendation.targetZone, recommendation.targetHrRange].filter(Boolean).join(" · ") || "Easy effort"}</p>
        </div>
      </div>
      <p className="relative mt-6 max-w-2xl text-sm font-semibold leading-7 text-white/78">Reason: {recommendation.reason}</p>
    </section>
  );
}
