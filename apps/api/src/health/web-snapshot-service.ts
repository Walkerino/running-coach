...
export async function getWebHealthSnapshot(input: { userId?: string; telegramId?: string }) {
  ...
  const webWorkouts = workouts.map((workout) => {
  ...
  const sleep = daily
    ...
    .map((item) => ({
      ...
      score: sleepScore(item.sleepMinutes),
  ...
  return {
    ...
    settings: {
      ...
      preferredEasyHrRange: `${user.usualEasyHrMin ?? 130}-${user.usualEasyHrMax ?? 140} bpm`,
    ...
    meta: {
      hasRealData: daily.length > 0 || workouts.length > 0,
